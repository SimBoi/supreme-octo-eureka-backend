...
function _login($conn, $database_name, $phone, $password)
{
    ...
    if (!mysqli_query($conn ,$sql)) throw new Exception('Failed to rehash password in database');
...
function signup($conn, $phone, $username)
{
    ...
    $sql = "SELECT Phone FROM Customers WHERE Phone = '".$phone."'";
    $result = mysqli_query($conn ,$sql);
    if(mysqli_num_rows($result) > 0) return '{"Result": "PHONE_EXISTS"}';
    $sql = "SELECT Phone FROM Teachers WHERE Phone = '".$phone."'";
    $result = mysqli_query($conn ,$sql);
    if(mysqli_num_rows($result) > 0) return '{"Result": "PHONE_EXISTS"}';

    // Insert the new customer into the database
    $sql = "INSERT INTO Customers (Phone, Username, CurrentAppointments, Orders, Coupons) VALUES ('".$phone."', '".$username."', '[]', '[]', '{}')";
    if (mysqli_query($conn, $sql)) $output = array('Result' => 'SUCCESS');
    else throw new Exception('Failed to insert new customer into database');
...
function update_profile($conn, $database_name, $phone, $password, $new_username) {
    ...
    if (strlen($new_username) < 1 || strlen($new_username) > 20) throw new Exception('Username is not between 1 and 20 characters long');
    ...
    if (!mysqli_query($conn ,$sql)) throw new Exception('Failed to update user profile in database');
...
function delete_account($conn, $phone, $password)
{
    ...
    if(mysqli_num_rows($result) > 0) {
        while($row = mysqli_fetch_assoc($result)) {
            ...
            if (!mysqli_query($conn ,$sql)) throw new Exception('Failed to delete user account from database');
